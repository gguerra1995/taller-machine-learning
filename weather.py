""" Taller Machine Learning , Dataset : weatherAUS.csv """
...
def mostrar_curvas_ROC(fpr, tpr, fpr2, tpr2, fpr3, tpr3, fpr4, tpr4, fpr5, tpr5):
    plt.figure(0).clf()
...
def mostrar_metricas(str_model, AUC, acc_validation, acc_test, y_test, y_pred, matriz_confusion):
    print('-' * 50 + '\n')
...
data.drop(['Date', 'Location', 'Sunshine', 'Evaporation', 'Cloud9am',
           'Cloud3pm', 'WindGustDir', 'WindDir9am', 'WindDir3pm'], axis=1, inplace=True)
...
matrixconfusion1, AUC, fpr1, tpr1, probs = matrix_confusion_AUC(
...
model, acc_validation, acc_test, y_pred, acc_train = metricas_entrenamiento(
    model, x_train, x_test, y_train, y_test)
matrixconfusion2, AUC, fpr2, tpr2, probs = matrix_confusion_AUC(
    model, x_test, y_test, y_pred)
...
model, acc_validation, acc_test, y_pred, acc_train = metricas_entrenamiento(
    model, x_train, x_test, y_train, y_test)
matrixconfusion3, AUC, fpr3, tpr3, probs = matrix_confusion_AUC(
    model, x_test, y_test, y_pred)
...
model, acc_validation, acc_test, y_pred, acc_train = metricas_entrenamiento(
    model, x_train, x_test, y_train, y_test)
matrixconfusion4, AUC, fpr4, tpr4, probs = matrix_confusion_AUC(
    model, x_test, y_test, y_pred)
...
model, acc_validation, acc_test, y_pred, acc_train = metricas_entrenamiento(
    model, x_train, x_test, y_train, y_test)
matrixconfusion5, AUC, fpr5, tpr5, probs = matrix_confusion_AUC(
    model, x_test, y_test, y_pred)
...
mostrar_curvas_ROC(fpr1, tpr1, fpr2, tpr2, fpr3, tpr3, fpr4, tpr4, fpr5, tpr5)
